//
//  TotalGameAPIManager.swift
//  QuizBay
//
//  Created by Srijan Kumar  on 12/08/22.
//

import UIKit

protocol TotalGameManagerDelegate {

    func updateAds(gameDetail: Int)

}


class TotalGameAPIManager: UIViewController {
    var delegate: TotalGameManagerDelegate?
            ...
            let task = session.dataTask(with: request){
                ...
                if let safeData = data{
                    ...
                    if let fetchedData = self?.parseJSON(safeData){
                        self?.delegate?.updateData(gameDetail: fetchedData)
                    }
                }
            }
            task.resume()
